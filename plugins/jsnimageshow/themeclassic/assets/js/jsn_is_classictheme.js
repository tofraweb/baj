 ...
 * @version $Id: jsn_is_classictheme.js 11387 2012-02-27 04:59:08Z trungnq $
...
var JSNISClassicTheme = {
	...
	ShowcaseChangeBg:function(){
		...
		$('imgpanel_bg_type').addEvent('change', function() {
			...
			switch (value) 
			{
				...
				case 'radial-gradient':
					...
					break;
				case 'pattern':
					$('jsn-bg-input-value').setStyle('float', 'none');
		...
		$('imgpanel_bg_type').fireEvent('click');
		$('imgpanel_bg_type').fireEvent('change');
		...
		var general_border_color = new MooRainbow('general_border_color_link', {
			id: 'generalbordercolor',
			imgPath: rainBowPath,
			startColor:JSNISClassicTheme.hextorgb($('general_border_color').value),
			onChange: function(color) {
		...
		var general_background_color = new MooRainbow('general_background_color_link', {
			id: 'generalbackgroundcolor',
			imgPath: rainBowPath,
			startColor:JSNISClassicTheme.hextorgb($('general_background_color').value),
			onChange: function(color) {
				$('general_background_color').value = color.hex;
				$('span_general_background_color').setStyle('background', color.hex);
			}
		});
	},
	...
	openLinkIn: function(elChangeID, elShowID, parent)
	{
		...
		{
			var wrapParent = $(parent).getParent();
			wrapParent.style.height = 'auto';
			...
			var wrapSize = wrapParent.getSize();
			wrapParent.style.height = wrapSize.y + 'px';
		});
	},
	...
	saveTabStatusCookie: function (cookieName)
	{
		...
		$$('.jsn-theme-panel').each(function(el)
		{
	...
	showcaseChangeImageMotionType: function(me)
	{
		...
		var wrapPanel = $('acc-image-presentation').getParent();
		wrapPanel.style.height = 'auto';
		
		if (me.value == 'no-motion') {
			zooming.style.display = 'none';
		} else {
			zooming.style.display = '';
		}
		var wrapSize = wrapPanel.getSize();
		wrapPanel.style.height = wrapSize.y + 'px';
	},
	
	fixDisplayNoMotion: function()
	{
		if ($('imgpanel_img_motion_type_expand').getProperty('value') == 'no-motion'){
			$('jsn-image-zooming-type').style.display = 'none';
		} else {
			$('jsn-image-zooming-type').style.display = '';
		}
	}
};