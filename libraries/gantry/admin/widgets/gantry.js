...
}}).post({model:"overrides",action:(GantryIsMaster)?"get_default_values":"get_base_values"});}};Gantry.Tips={init:function(){document.getElements(".g4-tooltips").twipsy({placement:"above-left",offset:{x:-10,y:-8}});
document.getElements(".hasTip").each(function(c){c.removeClass("hasTip").addClass("sprocket-tip");c.set("title",c.get("title").split("::").pop());c.twipsy({placement:"below-right",offset:{x:5,y:5},html:true});
});var a=$$(".g4-panel"),b;if(document.id(document.body).getElement(".defaults-wrap")){b=a.getElements(".g4-panel-left .gantry-field > label:not(.rokchecks), .g4-panel-left .gantry-field span[class!=chain-label][class!=group-label] > label:not(.rokchecks)");
}else{b=a.getElements(".g4-panel-left .gantry-field .base-label label");}b.each(function(c,d){if(c.length){c.addEvent("mouseenter",function(){var g=c.indexOf(this);
