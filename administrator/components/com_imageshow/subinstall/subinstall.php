 ...
 * @version $Id: subinstall.php 14165 2012-07-19 05:12:56Z giangnd $
...
class JSNSubInstaller extends JObject
{
    ...
    public function install()
	{
		...
		if (!$FTPEnable)
		{
			...
			$checkPlg 	= $this->_checkWritablePlugin();
			if ($checkPlg == true)
			{
				$resultCheckPlg = true;
			}
			...
			$checkEditorXtd 		= $this->_checkWritableEditorsXtd();
			if($checkEditorXtd == true)
			{
				$resultCheckEditorXtd = true;
			}
			else
			{
				$this->setError('plgeditor_xtd');
			}

			if ($resultCheckPlgContent == false || $resultCheckPlgSystem == false || $resultCheckModule == false || $resultCheckLangBO == false || $resultCheckLangFO == false || $resultCheckEditorXtd == false || $resultCheckPlg == false)
			{
				return true;
			}
		}
	...
	private function _checkWritablePlugin()
	{
		$filepath 	= JPATH_ROOT.DS.'plugins';
	...
	private function _checkWritableEditorsXtd()
	{
		$filepath 	= JPATH_ROOT.DS.'plugins'.DS.'editors-xtd';
		if (is_writable($filepath))
		{
			return true;
		}
