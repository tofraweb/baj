	...
	*/
	...
	function saveImages($data=array()){
		...
		if($number>1){
			$db = JFactory::getDBO();
			for($i=0;$i<$number;$i++){
				if($data["title"][$i]!=$data["originalTitle"][$i]||$data["description"][$i]!=$data["originalDescription"][$i]||$data["image_link"][$i]!=$data["originalLink"][$i]){
					$query = "UPDATE #__imageshow_images
							  SET image_title = ".$db->quote($db->escape($data["title"][$i]), false).",
								image_description = ".$db->quote($db->escape($data["description"][$i]), false ).",
