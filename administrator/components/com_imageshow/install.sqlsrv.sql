...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[#__imageshow_messages]') AND type in (N'U'))
BEGIN
CREATE TABLE [#__imageshow_messages](
    ...
    [published] [smallint] DEFAULT '0',
    [ordering] [int] DEFAULT '0',       
 CONSTRAINT [PK_#__imageshow_messages_msg_id] PRIMARY KEY CLUSTERED 
(
	[msg_id] ASC
)WITH (STATISTICS_NORECOMPUTE  = OFF, IGNORE_DUP_KEY = OFF)
)
...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[#__imageshow_parameters]') AND type in (N'U'))
BEGIN
CREATE TABLE [#__imageshow_parameters](
	[id] [int] IDENTITY(1,1) NOT NULL,
    [show_quick_icons] [smallint] DEFAULT '1',    
	[enable_update_checking] [smallint] DEFAULT '1',  
	[number_of_images_on_loading] [nvarchar](50) DEFAULT '30',	
 CONSTRAINT [PK_#__imageshow_parameters_id] PRIMARY KEY CLUSTERED 
(
	[id] ASC
)WITH (STATISTICS_NORECOMPUTE  = OFF, IGNORE_DUP_KEY = OFF)
)
