 ...
 * @version $Id: default_step3.php 14439 2012-07-27 03:32:43Z giangnd $
...
$returnAfterUpdate  		= ($return == '')?'index.php?option=com_imageshow':base64_decode($return);
...
if ($authentication)
{
	...
	{
		$mainframe->redirect('index.php?option=com_imageshow&controller=updater&return='.$return);
	}
}
?>
	<h2>
		<?php
		echo JText::_('UPDATER_HEADING_STEP2');
		?>
	</h2>
<div id="jsn-updater-upgrading">
	<?php if (count($sources) || count($themes) || $this->imageshowCore->needUpdate){ ?>
		...
		<div id="jsn-updater-successfully" class="jsn-installation-successfull-message">
			<?php echo JText::_('UPDATER_INSTALLATION_CONGRATULATION')?>
		</div>
		<div id="jsn-updater-buttons" class="form-actions">
			<?php
				...
				$dataUpdate['redirectLink'] = 'index.php?option=com_imageshow&controller=updater&return='.$return;
			...
			<script type="text/javascript">
				...
				function redirectAfterFinishInstall()
				{
					if (updateData.core.length > 0 && (themeData.length > 0 || sourceData.length > 0)){
						window.location.href = 'index.php?option=com_imageshow&controller=updater&step=3&return=<?php echo $return;?>';
					} else {
						window.location.href = '<?php echo $returnAfterUpdate.'&popup=0';?>';
					}
				}
			...
			<button id="jsn-updater-button-close" class="btn btn-primary" onclick="window.location.href='index.php?option=com_imageshow';"><?php echo Jtext::_('UPDATER_BUTTON_CLOSE');?></button>
			<button id="jsn-updater-button-finish" class="btn btn-primary" onclick="redirectAfterFinishInstall(); return false;"><?php echo Jtext::_('UPDATER_BUTTON_FINISH');?></button>
		</div>
	<?php } else if (!count($sources) && !count($themes) && !$this->imageshowCore->needUpdate && $this->canAutoUpdate) { ?>
		<?php $mainframe->redirect('index.php?option=com_imageshow&controller=updater&return='.$return);?>
	<?php } else {?>
		<?php $mainframe->redirect('index.php?option=com_imageshow&controller=updater&return='.$return);?>
	<?php }?>
</div>
