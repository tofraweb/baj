 ...
 * @version $Id: internal_source_folder.php 14353 2012-07-25 02:45:24Z giangnd $
...
class JSNInternalSourceFolder extends JSNImagesSourcesInternal
{
	...
	function drawTree($xmlObj, $path, $syncAlbum)
	{
		...
		{
			//don't include folder thumbnail to category "jsn_is_thumbs" => requirement from giangnd
	...
	public function loadImages($config = array())
	{
		...
		if (is_null(@$parameters->enable_update_checking) || (int) $parameters->number_of_images_on_loading == 0)
		{
			$num = 30;
		}
		else
		...
		if (count($matchesInvalidFolder) == 0) {
			$this->_error 	 = true;
			$this->_errorMsg = JText::_('SHOWLIST_FLEX_FOLDER_SOURCE_INVALID_FOLDER_NAME');
			return false;
		}
	...
	public function loadScript()
	{
		...
		$document->addScript(JUri::root(true).'/administrator/components/com_imageshow/imagesources/source_folder/assets/js/internal_source_folder.js?v='.@$componentVersion);
	}
