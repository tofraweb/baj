 ...
 * @version $Id: jsn_is_showcasetheme.php 14163 2012-07-19 03:38:31Z giangnd $
...
class JSNISShowcaseTheme
{
	...
	function getShowcaseThemeByShowcaseID($showcaseID, $URL)
	{
		...
		$modelShowcaseTheme = JModel::getInstance($themeProfile->theme_name);
	...
	function insertThemeProfile($themeID, $showcaseID, $themeName)
	{
		$query = 'DELETE FROM #__imageshow_theme_profile WHERE theme_id = '.(int)$themeID . ' AND showcase_id =' .(int)$showcaseID;
		$this->_db->setQuery($query);
		$this->_db->query();

		$query = 'INSERT INTO #__imageshow_theme_profile (theme_id, showcase_id, theme_name) VALUES ('.(int)$themeID.', '.(int)$showcaseID.', '. $this->_db->quote((string)$themeName) .')';
		$this->_db->setQuery($query);
	...
	/*function getNewestThemeVersion($themeObj)
	{
		$link		 	= JSN_IMAGESHOW_INFO_URL.'&identified_name='.urlencode($themeObj->element).'&edition='.urlencode($themeObj->edition);
		$objJSNHTTP   	= JSNISFactory::getObj('classes.jsn_is_httprequest', null, $link);
		$result    		= $objJSNHTTP->DownloadToString();

		if (!$result)
		{
			return (object) array('connection' => false, 'version' => '', 'commercial'=>'');
		}
		else
		{
			$objJSNJSON = JSNISFactory::getObj('classes.jsn_is_json');
			$parse = $objJSNJSON->decode($result);
			return (object) array('connection' => true, 'version' => @$parse->version, 'commercial' => @$parse->authentication);
		}
	}*/

	/*function checkNewestThemeVersion($themeObj)
	{
		$newVersion = $this->getNewestThemeVersion($themeObj);

		if ($newVersion->version == '') {
			return false;
		}

		$result = version_compare($themeObj->version, $newVersion->version);
	...
	/*function getThemeInfoUpgrade($themeElementName)
	{
		$themeInfo = $this->getThemeInfo($themeElementName);

		if ($themeInfo)
		{
			$themeObj = new stdClass();
			$themeObj->element 	= $themeElementName;
			$themeObj->edition 	= '';
			$themeObj->version  = $themeInfo->version;
			$themeObj->upgrade  = $this->checkNewestThemeVersion($themeObj);
			$themeObj->name     = $themeInfo->name;
			return $themeObj;
		}
