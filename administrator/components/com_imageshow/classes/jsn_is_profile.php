 ...
 * @version $Id: jsn_is_profile.php 14251 2012-07-23 04:05:08Z giangnd $
...
class JSNISProfile
{
	...
	function saveParameters($post)
	{
		...
		if (count($resultCheck) > 0)
		{
			$query = 'UPDATE #__imageshow_parameters
					  SET
					  	...
					  	enable_update_checking 	= '.(int) $post['enable_update_checking'].',
					  	number_of_images_on_loading 	= '.(int) $post['number_of_images_on_loading'].'
		...
		{
			$query = 'INSERT INTO #__imageshow_parameters (show_quick_icons, enable_update_checking, number_of_images_on_loading) VALUES ('.(int) $post['show_quick_icons'].', '.(int) $post['enable_update_checking'].', '.(int) $post['number_of_images_on_loading'].')';
			$this->_db->setQuery($query);
			$result = $this->_db->query();
