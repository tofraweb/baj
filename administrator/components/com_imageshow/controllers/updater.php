 ...
 * @version $Id: updater.php 14433 2012-07-27 02:31:11Z giangnd $
...
class ImageShowControllerUpdater extends JController
{
	...
	function install()
	{
		...
		$link = 'index.php?option=com_imageshow&controller=updater&type='.$type.'&element_id='.$elementID.'&commercial='.$commercial.'&return='.$return;
		...
		switch ($type)
		{
			...
			case 'theme':
				...
				{
					$this->setRedirect('index.php?option=com_imageshow&controller=updater&return='.$return);
				}
			break;
			default:
				$this->setRedirect($link);
	...
	function authenticate()
	{
		...
		$link			= 'index.php?option=com_imageshow&controller=updater&step=2&return='.$return;
		$msg			= '';
		$type			= '';
		...
		if ($result)
		{
			...
			else
			{
				...
				$this->setRedirect('index.php?option=com_imageshow&controller=updater&step=3&return='.$return);
				return true;
			}
		}
		else
