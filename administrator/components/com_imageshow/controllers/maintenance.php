 ...
 * @version $Id: maintenance.php 14386 2012-07-25 09:25:28Z giangnd $
...
//	}
	...
	function saveProfileParameter()
	{
		JRequest::checkToken() or jexit('Invalid Token');
		$post = JRequest::get('post');
		$params	= json_decode($post['profile_parameter']);
		$params->number_of_images_on_loading = $post['number_of_images_on_loading'];
		$params = json_encode($params);
		$query = 'UPDATE #__extensions SET params = \''.$params.'\' WHERE element = "'.$post['image_source'].'" AND folder = "jsnimageshow"';
		$db = JFactory::getDbo();
		$db->setQuery($query);
		$db->query();
		jexit();
