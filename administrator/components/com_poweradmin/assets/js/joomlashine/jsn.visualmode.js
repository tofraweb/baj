	...
	$.extend({
		_visualmode:{
					...
					baseUrl+'administrator/index.php?option=com_poweradmin&task=selectmoduletypes.setModuleType&pwadvisual=1&eid='+eid, 
					{
						...
						open: function(){
							...
							iframe.load(function(){
								...
								if(iframe.contents().find('input[id="jform_position"]').length > 0){
									iframe.contents().find('input[id="jform_position"]').val(position);
								}
			...
			moduleOptions: function(mid){
				var wWidth  = $(window).width()*70/100;
				var wHeight = $(window).height()*70/100;
            	var pop = $.JSNUIWindow
            	(
