...
(function($){
	...
	*/	
	$.JSNModulesGrid = function()
	{
		...
		* @return: build menu
		*/
		JSNGrid.buildModulesContentMenu = function(objs, rebuild){
		...
		JSNGrid.buildPositionContextMenu = function(objs){
			...
			if (context.isNew() ){
				...
				context.addItem( JSNLang.translate('JSN_RAWMODE_POSITION_CONTEXTMENU_VIEWPOSITIONS') ).addEventHandler("click", function(){
					...
					var pop = $.JSNUIWindow
					(
						baseUrl+'administrator/index.php?option=com_poweradmin&view=positionlisting&format=raw',
						{
							modal : true,
				...
				baseUrl + 'administrator/index.php?option=com_poweradmin&task=modules.loadModulesJsonData&lang='+lang, 
				{
					...
					if ( data != '' && data != null ){
						...
						$.each( data.modules, function( i, item ){
								...
								}).html('<div class="poweradmin-module-item-inner"><div class="poweradmin-module-item-inner-text">'+$.unhtmlspecialchars(item.title)+'</div><div class="clearbreak"></div></div><div class="poweradmin-module-item-drag-handle"></div><div class="clearbreak"></div>').appendTo(objPos);
						...
						setTimeout(function(){
							...
							JSNGrid.buildModulesContentMenu( JSNGrid.grid.modules.getAllFromParent( objPos ), false );
		...
		*/
		JSNGrid.load = function(backgroundLoad){
			...
			$.when(
				...
				JSNGrid.buildModulesContentMenu(  JSNGrid.grid.modules.getAll() )
			).then(
				$('body').showLoading({removeall:backgroundLoad})
			);
		};
