				...
				$this.intervalFilter = setInterval(function(){
					...
					if ($this.timeShowResult == 60){
						...
						if (filter){
							...
							positions_listing.find("div:not(contains("+filter+"))").parent().addClass('mark_hidden');
	...
	{		
		...
		this.initFilter	= function ()
		{
			...
			input.keyup( function (){						
				//setInterval('doFilter()',300);
				if(this.value && this.value.length >= 3){
					inputContainer.children('.close').show();
					_root.doFilter(this.value);
				}else if(!this.value){
					_root.resetFilter();
		...
		this.doFilter	= function (filterValue)
		{
