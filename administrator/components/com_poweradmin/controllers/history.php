...
# @version $Id: history.php 12506 2012-05-09 03:55:24Z hiennh $
-------------------------------------------------------------------------*/
 ...
 * @author binhpt
 */
class PoweradminControllerHistory extends JController
{
	...
	public function load()
	{
		...
		$dbo->setQuery("SELECT * FROM #__poweradmin_history WHERE user_id={$user->id} ORDER BY visited DESC LIMIT {$limit}");
	...
	public function save()
	{
		...
		$query->select('id')
			->from('#__poweradmin_history')
			->orderBy('visited DESC');

		$dbo->setQuery($query, 0, $limit);
		$ids = $dbo->loadResultArray();
		
		$dbo->setQuery("DELETE FROM #__poweradmin_history WHERE id NOT IN(".implode(', ', $ids).")");
		$dbo->query();
