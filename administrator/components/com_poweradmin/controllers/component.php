...
class PoweradminControllerComponent extends JControllerForm
{
		...
		switch ( $data->get('requestType', 'only') )
		{
			...
			case 'globally':
				...
				$allMenuitems = $this->getModel('menuitem')->getAllItems(
					array(
						'option' => $data->get('option', 'com_content'),
	...
	{
		$scriptFolder = JString::strtolower(JRequest::getCmd('scriptFolder', ''));
		$scriptName   = JString::strtolower(JRequest::getCmd('scriptName', ''));
		$scriptFile   = JSN_POWERADMIN_PATH.DS.'assets'.DS.'js'.DS.'joomlashine'.DS.'components'.DS.$scriptFolder.DS.$scriptName.'.js';
		if ( JFile::exists($scriptFile) ){
			$found = JSN_POWERADMIN_LIB_JSNJS_URI.'components/'.$scriptFolder.'/'.$scriptName.'.js';
		}else{
			$found = 'Not Found';
		}
		jexit($found);
