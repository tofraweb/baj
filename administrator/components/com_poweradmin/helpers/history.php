...
# @version $Id: history.php 13308 2012-06-14 09:33:05Z binhpt $
-------------------------------------------------------------------------*/
...
final class PowerAdminHistoryHelper
{
	...
	private static function updateHistoryState($post)
	{
		...
		$dbo = JFactory::getDBO();
		$dbo->setQuery("UPDATE #__poweradmin_history SET is_deleted={$isDelete} WHERE list_page_params LIKE '{$listPage->params}' AND object_id IN (".implode(',', $id).")");
		$dbo->query();
	}
