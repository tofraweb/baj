...
{	
	...
	public function getItems ()
	{
		...
		foreach ($results as $record) {
			...
			if (isset($record->manifest_cache)) {
				...
				switch ($this->context) {
					...
					case 'modules':
						...
						$item->description = $this->_description($record->position);
					break;
					...
					case 'templates':
						...
						$item->description = $this->_description(JText::_(strtoupper($manifest->description),true));
					break;
				}
			}
			elseif (!isset($mapping['fields'])) {
				...
				$item->title = $this->_title(preg_replace('/\{([^\}]+)\}/ie', "\"{\$record->\\1}\"", $mapping['fields']['title']));
				$item->description = $this->_description(preg_replace('/\{([^\}]+)\}/ie', "\"{\$record->\\1}\"", $mapping['fields']['description']));
			}
			
			$item->link = preg_replace('/\{([^\}]+)\}/ie', "\"{\$record->\\1}\"", $mapping['link']);
			$items[] = $item;
		}
	...
	private function _getTableMapping ($name) {
		...
		if (intval($params->get('search_trashed', 0)) == 0)
		{
			...
			$mappings['com_k2']['conditions'] 					= 'trash=0';
			$mappings['com_k2_categories']['conditions'] 		= 'trash=0';
	...
	private function _description ($description) {
		...
		$coverage = $this->getState('search.coverage');
		if (isset($this->_descriptionMap[$coverage]) && !empty($description)) {
			$description = str_replace('{desc}', $description, $this->_descriptionMap[$coverage]);
		}
