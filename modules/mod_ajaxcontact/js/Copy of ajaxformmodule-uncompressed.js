...
if(typeof(jQuery) != "undefined" && $===jQuery){
	...
	var ajaxModule = new Class({
  	...
  	initialize: function(options){
		this.setOptions(options);
	  	...
	  	this.attachEventToSubmitButton();
	},
	...
	attachEventToSubmitButton:function(){
		var obj	= this;
		if(document.formvalidator){
			// Iterate through the form object and attach the validate method to all input fields.
			$A($(this.options.frm).elements).each(function(el){
				el = $(el);
				if ((el.get('tag')  == 'input' || el.get('tag') == 'button') && el.getProperty('type') == 'submit') {
					if (el.hasClass('validate')) {
						el.onclick = function(){
							if(!document.formvalidator.isValid($(obj.options.frm))){
