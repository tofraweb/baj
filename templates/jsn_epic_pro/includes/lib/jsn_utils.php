 ...
 */
	...
	class JSNUtils {
		...
		function arrangeFileInHeadSection(&$header_stuff, $firstOrdering)
		{

			// remove modal.js, core.js, mootools-core.js in HeaderData to prevent double load
			unset($header_stuff['scripts'][JURI::root(true).'/media/system/js/mootools-more.js']);
			unset($header_stuff['scripts'][JURI::root(true).'/media/system/js/modal.js']);
			unset($header_stuff['scripts'][JURI::root(true).'/media/system/js/core.js']);

			// arrange file
			$data  =& $header_stuff['scripts'];
			if (count($data))
			{
				foreach ($firstOrdering as $topScript) {
					if (array_key_exists($topScript, $data)) {
						$first[$topScript] = $data[$topScript];
						unset($data[$topScript]);
					}
				}
				$data = $first + $data;
			}
