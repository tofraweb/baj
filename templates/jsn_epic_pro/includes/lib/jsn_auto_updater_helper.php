...
class JSNAutoUpdaterHelper extends JSNUpdaterHelper
{
	...
	function downloadTemplatePackage($post)
	{
		...
		$link					= JSN_TEMPLATE_AUTOUPDATE_URL.'&identified_name='.urlencode($this->_identified_name).'&based_identified_name='.urlencode($this->_based_identified_name).'&edition='.urlencode($this->_edition).'&joomla_version='.urlencode($this->_joomla_version).'&username='.urlencode($post['customer_username']).'&password='.urlencode($post['customer_password']).'&upgrade=yes';
