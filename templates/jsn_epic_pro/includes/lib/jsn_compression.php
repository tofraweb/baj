...
class JSNCompression
{
	...
	function __construct($type, $file_list, $compress_options)
	{
		...
		$this->__set('metadata_filename', 	$this->compress_options['cache_path'].DS.$this->compress_options['template_name'].'_'.$type.'_metadata.json');
		$this->__set('compress_filename', 	$this->compress_options['cache_path'].DS.$this->compress_options['template_name'].'_'.$type.'_'.$this->compress_options[$type.'_active_profile'].'.'.$type);
		...
		{
			$this->processed_file_list[] = $value['file_abs_path'].DS.$value['file_name'];
		}
	}
	/**
	 * Set method
	...
	function process()
	{
		...
		{
			case 'css':
				require_once 'compression'.DS.'jsn_css_compression.php';
				$compression_engine = new JSNCssCompression();
				$uri = JURI::root(true);
				foreach($this->file_list as $key => $value)
				{
					/*if ($uri != '' && substr_count($key, $uri) == 0)
					{
						$key = $uri.$key;
					}*/
					if (!(file_exists($this->compress_filename) && @filemtime($value['file_abs_path'].DS.$value['file_name']) < @filemtime($this->compress_filename)))
					{
						 $need_update = true;
						 break;
					}
				...
				if ($need_update)
				{
					foreach($this->file_list as $key => $value)
					{	
						$css_content 		 = file_get_contents($value['file_abs_path'].DS.$value['file_name']);
			...
			case 'js':
				...
				if ($need_update)
				{
					foreach ($this->file_list as $key => $value)
					{
						if ($value['file_name'] && !isset($value['first']))
						{
							$compress_content .= file_get_contents($value['file_abs_path'].DS.$value['file_name']);
						}

						if (isset($value['first']))
						{
							$first_script .= file_get_contents($value['file_abs_path'].DS.$value['file_name']);
						}
					}
					$compress_content = $first_script.$compress_content;
				}
				break;
		}
