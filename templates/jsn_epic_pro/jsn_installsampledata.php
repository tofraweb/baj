...
# @version $Id: jsn_installsampledata.php 14646 2012-07-30 12:08:32Z giangnd $
-------------------------------------------------------------------------*/
			...
			<div id="jsn-page_inner1">
				<div id="jsn-page_inner2">
					<h1><?php echo JText::sprintf('JSN_SAMPLE_DATA_MAIN_TITLE', $template_manifest['name_uppercase']); ?>
						<span class="action-cancel"><a id="jsn-install-cancel" class="link-action" href="javascript: void(0);" onclick="window.top.setTimeout('SqueezeBox.close();', 100);"><?php echo JText::_('JSN_SAMPLE_DATA_CANCEL_TEXT'); ?></a></span>
					</h1>
						...
						<?php if (!$session->get($identifier, false, 'jsntemplatesession')) { ?>
								...
								<?php } else { ?>
									<div id="jsn-sampledata-installation">
										...
										<script type="text/javascript">
											window.addEvent('domready', function() {
												JSNSampleData.start('<?php echo strtolower($template_manifest['full_name']);?>', '<?php echo JURI::root();?>', <?php echo $template_style_id; ?>);
											});
										</script>
