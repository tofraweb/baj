...
include_once(dirname(__FILE__).DS.'jsn_archive.php');
require_once(dirname(__FILE__).DS.'jsn_readxmlfile.php');
include_once dirname(__FILE__).DS.'jsn_installer.php';
require_once(dirname(__FILE__).DS.'jsn_checksum_integrity_comparison.php');
require_once(dirname(__FILE__).DS.'jsn_checksum_file_comparison.php');
class JSNUpdaterHelper
{
	...
	function createZip($file)
	{
		...
		$fileName 			= JPATH_ROOT.DS.'tmp'.DS.date('Ymd_H\hi\ms\s').'_'.$this->_template_folder_name.'_'.$version.'.bak.zip';
	...
	function downloadFile($type, $file_name, $upgrade = false)
	{
		jimport('joomla.filesystem.file');
		if ($upgrade === true)
		{
			$file_path = JPATH_ROOT.DS.'tmp'.DS.$file_name;
		}
		else
		{
			$file_path = $this->_template_folder_path.DS.'backups'.DS.$file_name;
		}
