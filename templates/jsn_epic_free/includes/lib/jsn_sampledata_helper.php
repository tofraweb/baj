...
# @version $Id: jsn_sampledata_helper.php 14688 2012-07-31 07:33:05Z sonlm $
-------------------------------------------------------------------------*/
		...
		if ($unpack_package)
		{
			...
			if ($installed_data && is_array($installed_data))
			{
				...
				else
				{
					...
					{
						$this->deleteRecordAssetsTableByName($isNotExsited);
					}
		...
		else
		{
			...
			if ($extManifestInfo['version'] != $extLatestVersion)
			{
				...
				else
				{
					...
					{
						/**
						 * PRO edition outdated, no update but need to be marked
						 * Only extension with sampledata will be warned!
						 */
						$finalMessage = JText::sprintf('JSN_SAMPLE_DATA_WARNING_EXT_PRO_EXIST', $extInfo->description);
						// Include the internal update link
						$internalUpdateLink = JURI::root() . 'administrator/index.php?option=com_' . $extInfo->name . '&controller=updater';
						$finalMessage .= '&nbsp;<a class="link-action" target="_blank" href="' . $internalUpdateLink . '">' . JText::_('JSN_SAMPLE_DATA_EXT_INTERNAL_UPDATE') . '</a>';
					}
				}
			}
		}
		$result = new stdClass();
		$result->needInstall  = $needInstall;
